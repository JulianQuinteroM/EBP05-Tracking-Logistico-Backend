    ...
    <section className="overflow-x-auto rounded-xl bg-white shadow-sm">
      <table className="w-full min-w-[850px] text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-100">{data?.content.map(item => <tr key={item.id}>
          ...
          <td className="px-4 py-3"><div className="flex gap-2"><Link href={`/envios/detalles?id=${item.id}`} className="rounded border border-purple-300 px-2 py-1 text-purple-900">Ver detalles</Link>{item.status === 'PENDIENTE_RECOGIDA' && <><Link href={`/envios/detalles?id=${item.id}&accion=editar`} className="rounded border px-2 py-1">Editar</Link><Link href={`/envios/detalles?id=${item.id}&accion=cancelar`} className="rounded border border-red-300 px-2 py-1 text-red-700">Cancelar</Link></>}</div></td>
        </tr>)}</tbody>
      </table>
